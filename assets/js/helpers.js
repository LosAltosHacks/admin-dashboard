...
async function signup(user) {
  if (
    ...
    !user.gender ||
    !user.tshirt_size ||
    !user.previous_hackathons
  )
    return false;
  else if (
    user.age < 18 &&
    (!user.guardian_name || !user.guardian_email || !user.guardian_phone_number)
  )
    return false;
...
async function modify(user_id, params) {
  let result = await request(
    "POST",
    "/registration/v1/modify/" + user_id,
    params
  );
  return result;
}
...
export async function getUser(query) {
  let result = await request("POST", "/registration/v1/search", {
    query: query
  });
  return result;
}
